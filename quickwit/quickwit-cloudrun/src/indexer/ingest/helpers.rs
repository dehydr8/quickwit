...
pub(super) async fn send_telemetry() {
    ...
    let telemetry_info =
        QuickwitTelemetryInfo::new(services, HashSet::from_iter([QuickwitFeature::GCPCloudRun]));
...
pub(super) async fn configure_source(
    ...
    Ok(SourceConfig {
        source_id: CLOUDRUN_SOURCE_ID.to_owned(),
...
pub(super) async fn init_index_if_necessary(
    ...
    let metadata = match metadata_result {
        Ok(metadata_resp) => {
            ...
            if !current_metadata.sources.contains_key(CLOUDRUN_SOURCE_ID) {
...
/// Prune old CloudRun file checkpoints if there are too many
///
...
/// than QW_CLOUDRUN_MAX_CHECKPOINTS. When this purging is performed, the CloudRun
/// indexer might ingest the same file again if it receives a duplicate
/// notification.
pub(super) async fn prune_cloudrun_source(
    metastore: &mut MetastoreServiceClient,
    index_metadata: IndexMetadata,
) -> anyhow::Result<()> {
    let cloudrun_checkpoint_opt = index_metadata
        .checkpoint
        .source_checkpoint(CLOUDRUN_SOURCE_ID);

    if let Some(cloudrun_checkpoint) = cloudrun_checkpoint_opt {
        if cloudrun_checkpoint.num_partitions() > *MAX_CHECKPOINTS {
            info!(
                partitions = cloudrun_checkpoint.num_partitions(),
                "prune CloudRun checkpoints"
            );
            metastore
