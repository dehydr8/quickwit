...
use crate::indexer::ingest::helpers::{prune_cloudrun_source, wait_for_merges};
...
pub async fn ingest(args: IngestArgs) -> anyhow::Result<IndexingStatistics> {
    debug!(args=?args, "cloudrun-ingest");
...
mod tests {
    ...
    async fn put_object(
        ...
        let src_location = format!("gs://{}/{}", bucket, prefix);
    ...
    async fn test_ingest() -> anyhow::Result<()> {
        ...
        let prefix = new_coolid("cloudrun-ingest-test");
        let storage_resolver = StorageResolver::unconfigured();

        let index_config = br#"
        version: 0.8
        index_id: cloudrun-test
        doc_mapping:
            field_mappings:
              - name: timestamp
                type: datetime
        ...
        unsafe {
            ...
            std::env::set_var("QW_CLOUDRUN_METASTORE_PREFIX", &prefix);
            std::env::set_var("QW_CLOUDRUN_INDEX_PREFIX", &prefix);
            std::env::set_var("QW_CLOUDRUN_INDEX_CONFIG_URI", config_uri.as_str());
        ...
        unsafe {
            ...
            std::env::remove_var("QW_CLOUDRUN_METASTORE_PREFIX");
            std::env::remove_var("QW_CLOUDRUN_INDEX_PREFIX");
